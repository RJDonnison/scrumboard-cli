...
async function exportBoard({ details = false, out, headed = false } = {}) {
  ...
  try {
    ...
    return await writeExport({ tasks, stories }, out);
  } finally {
    await browser.close();
  }
}
...
/**
 * Finds a single task card by its title (case-insensitive, exact match)
 * among all columns on the board.
 *
 * @param {import("playwright").Page} page
 * @param {string} name
 * @returns {Promise<import("playwright").Locator>}
 */
async function findTaskCard(page, name) {
  const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const card = page.locator(".task-card").filter({
    has: page.locator(".task-name strong", {
  ...
  if (count > 1) {
    throw new Error(
      `Found ${count} tasks named "${name}" — names aren't unique on this board, so this command can't tell them apart.`,
    );
  }
...
async function scrapeTaskDetails(page, card, includeAcs) {
  ...
  const acs = includeAcs
    ? await scrapeACs(panel, parseCoveredACs(description))
    : [];
...
/**
 * Parses the AC labels referenced in a task's "ACs covered: ..." line.
 *
 * @param {string} description - Full task description text.
 * @returns {string[]} Referenced AC labels, e.g. ["AC1", "AC2"]. Empty if
 *   the description has no "ACs covered" line.
 */
function parseCoveredACs(description) {
  const match = description.match(/ACs?\s+covered:\s*(.+)/i);
  if (!match) return [];
...
async function exportTask(
  ...
  try {
    ...
    return writeExport({ task }, out);
  } finally {
    await browser.close();
  }
}

module.exports = { exportBoard, exportTask };
